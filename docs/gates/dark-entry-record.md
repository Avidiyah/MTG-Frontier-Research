...
- Status: **entry record; audit not open — technical items pending Phase 3
  freeze; independent roles and program-owner authorization outstanding**
...
| 3 | Measurement-freeze commit, data snapshots, CR, protocol, guide, preregistration, and five earlier export hashes recorded. | Freeze commit `70fa956`. Protocol `1bc05d357b…`, guide `d31dee0a3b…`. Snapshot manifest `docs/manifests/snapshot-scryfall-2026-08-25.json`. Earlier export hashes: lea `aabc1bd5…`, leb `4cb90170…`, arn `4827f5be…`, atq `8ec1047b…`, leg `c39a2d69…`. Preregistration hash and the §3 frozen-input block are recorded at Phase 3 freeze. | **partial** — completed by Phase 3 §3 population |
...
| 5 | A held-out-safe deterministic development export exists and is verified by aggregate counts only. | Produced by `verify_export_safety.py drk --runs 2` + retained `export_units.py drk --exclude-heldout` in Phase 3. | **pending** |
| 6 | The cumulative held-out exclusion registry, including the four named incident exclusions, is bound to the audit. | Protocol §6.3 pool (digest `377e12bd…`, snapshot manifest) plus Combust, Malignus, Lava Burst, Wild Slash and every other logged incident; bound in the Dark experiment manifest at Phase 3. | **partial** — registry named; manifest binding in Phase 3 |
| 7 | Both independent annotators and the adjudicator are assigned. | §3 role blocks. | **pending** |
| 8 | Neither annotator has inspected eligible Dark text before the freeze. | §3 attestations. | **pending** |
...
**Readiness statement:** items 1, 2, 4, 9 are satisfied; items 3 and 6 are
partial and completed by the Phase 3 input freeze; items 5, 7, 8, 10 are
pending. The Dark audit is **not authorized to open**.
