...
- Status: **adjudicated and closed 2026-08-28 (§6). Historical: pass 1 sealed
  and preserved under the §3.4 assignment-timing reconciliation; pass 2 sealed
  2026-08-27; the separate adjudicator opened only after both seals were
  verified**
...
## 6. Adjudication record and closure

### 6.1 Adjudicator pre-access attestation

```text
THE DARK ADJUDICATOR PRE-ACCESS ATTESTATION
Adjudicator identity:      fresh-dark-adjudicator-2026-08-27
Session opened:            2026-08-27, after commit 61a7530 ("Pass 2 The Dark")
Invocation note:           the session was addressed as
                           "fresh-dark-adjudicator-2026-05-27"; the recorded
                           assignment identity (§3.6) is used throughout and the
                           string difference is treated as a typographical error
Order of access:           (1) control documents, agreement tooling, and the
                           Legends closure precedent; (2) SHA-256 verification of
                           the sealed files against §3.6/§5 and the retained
                           export against preregistration §3 — all equal;
                           (3) the §7.3 alignment/agreement report generated by
                           audit_metrics.py --compare; (4) both sealed passes.
                           No Dark row was opened before step (2) passed.
Held-out exposure:         none; card lookups were made only by exact name or
                           Oracle ID of cards present in the frozen export, plus
                           one committed lea development card (Gaea's Liege) for
                           the guide example-19 comparison
Classifier proposals:      none made during adjudication; P-LEG-1..3 not
                           implemented; the findings' S10 items were written
                           after the final annotation file was hashed
```

### 6.2 Alignment and agreement (preregistration §7.3)

- Both passes: 163 rows, 163 unique `(oracle_id, face, index)` keys, no
  missing or duplicate keys, structural drift 0 against each other and against
  the retained export.
- Exact judgement-record agreement **141 / 163 (0.8650)** — below H10's 0.95;
  exact-card agreement **89 / 110 (0.8091)**.
- Disagreements: 22 rows; 18 differ only on `context` (`none` vs
  `type_line`, plain instant/sorcery spell text); Runesword #0/#1, Angry Mob
  #1, and Leviathan #1 differ substantively. `role_ok` and `source_ok` had no
  disagreement.

### 6.3 Adjudication outcome

- Union reviewed: 23 rows (22 disagreements + consensus defect Venom #1); each
  carries an `ADJ:` rationale. Results: the 18 context rows → `none`;
  Runesword #0 `over`/`defect` and #1 `under` (`missed 0`)/`defect`
  (representable non-contiguous-parent shape, not a schema gap); Angry Mob #1
  `ambiguous` (frozen guide example 19); Leviathan #1 `accept` (CR 113.2c);
  Venom #1 consensus `defect`.
- Final dispositions 159 accept / 3 defect / 1 ambiguous / 0 unsupported /
  0 adjudicate. All 163 rows retain both pass IDs plus the adjudicator ID.

### 6.4 Sealing record

```text
Path:                       docs/audits/drk/units-annotated.tsv
Sealed:                     2026-08-28T00:00:20-05:00
SHA-256:                    aed8ab6309f7ad679c1a5e455c1d9a2d6567c7ad83121175c334480b80d25730
Rows / unique stable keys:  163 / 163; drift 0 against the retained export
Metrics:                    docs/audits/drk/metrics.json
                            sha256 8c02d616c63f225f2695d29736a48ad1e85f59c31b7278d9c6f1dae604dade65
Checks:                     python unittest 21 passed; verify_manifests.py valid
                            (snapshot + dark-freeze manifests); no source, export,
                            protocol, guide, or sealed pass file modified
Findings:                   docs/findings/drk-structural-audit.md (populated)
Status:                     The Dark is adjudicated and closed; H10 falsified and
                            reported; next eligible set Fallen Empires (fem)
```
