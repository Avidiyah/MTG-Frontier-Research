...
- Decision basis: repository state at `2355b6c` (P-ATQ technical acceptance
  evidence merged at `bcf9eaa`) plus the P-ATQ research acceptance report;
  the technical P-ATQ package is complete
- Companion record: `docs/gates/legends-entry-record.md` (Legends entry
  checklist completion record and role templates)
...
| Frozen manifest | Pool rule and snapshot-specific 2,096-identity digest are frozen in `docs/manifests/snapshot-scryfall-2026-08-25.json`; the structural held-out sample and gold manifest are not. | **partial** | Gate 1 sample/gold stable-key manifests and hashes before parser tuning. |
...
|---|---|---|---|---|
| P-ATQ technical acceptance package | **satisfied** — research acceptance (`docs/findings/p-atq-research-acceptance-assessment.md`, status "research and technical S10 acceptance complete") and the technical package (`docs/audits/corpus-checks/2026-08-26-post-patq-merge.md`, `2026-08-26-patq-s8-search.md`; S10 decisions in `docs/findings/atq-structural-audit.md` §8) are merged at `bcf9eaa`. The one evidence-record correction (P-ATQ-3: five kind corrections plus three correct positives, not eight misfires) is recorded explicitly; no disposition was changed silently. | None. Reopen only an affected proposal on a concrete counterexample. | Research lead adjudicates any future conflict. | **No.** |
| Live baseline reconciled in `docs/current-state.md` | **satisfied** — the main body states the accepted baseline (71,563 / 37,299 / 861 + 30 / 166) and P-ATQ-1..4 as accepted; the four pre-acceptance log entries carry explicit supersession markers (reconciliation commit after `2355b6c`, see `docs/gates/legends-entry-record.md`). | Re-verify at the freeze commit that `docs/current-state.md` and `2026-08-26-post-patq-merge.md` still agree. | Technical measurement owner; research lead verifies. | **No.** |
| Legends preregistration and empty outline | **satisfied** by `docs/findings/leg-structural-audit-preregistration.md` and `docs/findings/leg-structural-audit.md`; countersignature **missing**. | Program-owner countersignature (P-ATQ has closed); sign-off text in `docs/gates/legends-entry-record.md` §5.3. | Research lead; program owner. | **Yes until countersigned.** |
| Held-out-safe development export | **technical capability satisfied; final audit input pending** — protocol T7 is implemented at the database query boundary for `cards`, database-backed `segment`, and native audit export; the TSV exporter validates the native exclusion attestation before writing rows. `docs/gates/pre-legends-technical-entry-evidence.md` records two byte-identical runs per format, 17 identities excluded, 0 held-out export records, and 426 / 426 unique stable keys without displaying rows. The row-bearing output was deliberately not retained before annotator attestations/freeze. | At the chosen clean freeze commit, after role attestations, rerun the same aggregate-only verifier, retain the resulting TSV as the frozen annotation input, and bind its hash into preregistration §3. No classifier change is involved. | Technical measurement owner; research lead verifies. | **Yes until the final frozen TSV is retained and bound.** |
| Frozen source/build/rules/export identities | **partial** — `docs/manifests/snapshot-scryfall-2026-08-25.json` now binds all three bulk files, `cards.sqlite`, CR, and the held-out pool digest; `docs/manifests/experiment-pre-legends-export-gate-2026-08-26.json` binds the technical verification's code, commands, environment, and output hashes. The clean freeze commit, final retained TSV, guide binding, and preregistration §3 block remain unset. | At the freeze commit, validate the manifests, record the clean commit/build and guide hash, retain the export, and populate preregistration §3 verbatim. | Technical measurement owner; research lead. | **Yes.** |
| Two independent annotators and adjudicator assigned | **missing** in current artifacts. | Assign identified roles using the templates in `docs/gates/legends-entry-record.md` §5 and collect both independence attestations before the freeze. | Research lead nominates; program owner approves. | **Yes.** |

## 6. Readiness conclusion

- **Ready to preregister:** yes.
- **Ready to inspect Legends:** no. P-ATQ closure and the T7 technical export
  gate are complete; the remaining pre-Legends entry dependencies (final
  retained export at the clean freeze, frozen-inputs block,
  annotator/adjudicator assignment and attestations, program-owner
  authorization) are open — see `docs/gates/legends-entry-record.md`.
