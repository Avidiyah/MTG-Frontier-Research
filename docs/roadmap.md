...
| D15 | Retract split rule (c): never split at a trigger-condition comma or cost colon or inside quotes; keep the unit whole and record the delayed trigger as a slot/signal (P-ATQ-1) | Implemented 2026-08-26 (`bf9eb04`); accepted 2026-08-26 under S10 — isolated S11: 982 → 861 children (all 121 comma/colon removed, 0 added), 0 in-quote splits; five fix rows re-annotated `under` (D15 slot), regression drift 0. Record: `docs/findings/atq-structural-audit.md`. | Accepted | research lead |
...
| D17 | Strip ability-word / Saga-chapter / named-mode prefixes before kind classification (P-ATQ-3) | **Accepted within its measured scope 2026-08-26**: 3,572 firings, 624/624 chapters triggered, 1,786 hidden trigger words recovered, binary cross-check 0 mismatches; Urza's Science Fair Project, Khârn the Betrayer and Diamond Weapon are correct prevention positives. Bounded counterexamples remain recorded (Prototype/spree/table-label prefixes; 2 flavor words with sentence punctuation; 141 new `keyword_ability` labels on funny/token products via `is_keyword_line`). | Complete, bounded | Codex; research lead |
