...
| D15 | Retract split rule (c): never split at a trigger-condition comma or cost colon or inside quotes; keep the unit whole and record the delayed trigger as a slot/signal (P-ATQ-1) | `docs/audits/corpus-checks/2026-08-26-delayed-split-overseg.md`; 5 `over` rows in lea/arn/atq | Phase 1, via S10 | Codex |
| D16 | Exclude `can't be prevented` statics from `prevention_effect` (P-ATQ-2) | kind-rules check A: 9 misfires | Phase 1, via S10 | Codex |
| D17 | Strip ability-word / Saga-chapter / named-mode prefixes before kind classification (P-ATQ-3) | kind-rules check A: 8 prefixed triggers labelled prevention; wider effect unmeasured | Phase 1, needs S8/S11 | research lead → Codex |
| D18 | Spell-only delayed triggers on instant/sorcery faces get role `delayed_trigger` (P-ATQ-4) | kind-rules check B: 30 units | Phase 1, needs S8 for off-stack exceptions | research lead → Codex |
