...
| D5 | Generic delayed-trigger detection for inverted, scoped `When …`-in-effect, reflexive `When you do`, and `at end of combat` forms (F1–F4) | Implemented 2026-08-26 via P-ARN-1/2; sentence-level rules ratified in lead review. The single-sentence comma/colon rule (c) was rejected (S11: 121 fragment parents, 3 in-quote splits) — see D15. | Partially complete | Codex |
| D15 | Retract split rule (c): never split at a trigger-condition comma or cost colon or inside quotes; keep the unit whole and record the delayed trigger as a slot/signal (P-ATQ-1) | **Accepted 2026-08-26** under S10 (`docs/audits/corpus-checks/2026-08-26-post-patq-merge.md` §4, §8): 982 → 861 sentence-level children, 121 → 0 comma/colon fragments, 3 → 0 in-quote splits; regression drift is exactly the five rule-(c) rows (Cockatrice, Thicket Basilisk, Rukh Egg, Nafs Asp, Battering Ram), whose `under` re-annotation is the lead's remaining bookkeeping. | Complete | Codex; re-annotation: research lead |
| D16 | Exclude `can't be prevented` statics from `prevention_effect` (P-ATQ-2) | **Accepted 2026-08-26** under S10 (report §5): 35 prohibition units corpus-wide, 0 labelled `prevention_effect`, 0 genuine prevention excluded, 181 → 166 `prevention_effect`. | Complete | Codex |
| D17 | Strip ability-word / Saga-chapter / named-mode prefixes before kind classification (P-ATQ-3) | Corpus-validated 2026-08-26 (report §6): 3,572 firings, 624/624 chapters triggered, 1,786 hidden trigger words recovered, binary cross-check 0 mismatches; bounded counterexamples recorded (Prototype/spree/table-label prefixes; 2 flavor words with sentence punctuation; 141 new `keyword_ability` labels on funny/token sets via `is_keyword_line`). **Acceptance pending Codex's adjudication of the 3 residual prevention rows** (Urza's Science Fair Project, Khârn the Betrayer, Diamond Weapon). | Implemented, pending adjudication | Codex (adjudication) → research lead |
| D18 | Spell-only delayed triggers on instant/sorcery faces get role `delayed_trigger` (P-ATQ-4) | **Accepted 2026-08-26** under S10 (report §7): full 12,466-unit I/S-face S8 sweep; 30 positives (0 false positives), 85 negatives inspected; bounded out-of-pattern classes recorded (Ertai's Meddling's recurring no-duration trigger; 45 whole-unit inverted cantrip lines; 9 comma-led / sentence-initial forms belonging to D5/D19). | Complete | Codex |
| D19 | Unscoped `When …` after effect text inside activated/triggered abilities (Tawnos's Coffin, Animate Dead) | atq V2; must be separated from D14's independent triggers by an S8 search | Phase 1 | research lead |
| D20 | Novelty breakdown by kind and cost form; N1 restated | atq V6: novelty rose to 96/125 despite a larger earlier pool | Phase 1 measurement | research lead |
| D21 | Export format for non-contiguous parent spans (Rocket Launcher class) and the native TSV export (T2 close-out) | atq V4; script vs native agreement 125/125 on atq | Phase 1 (tooling) | Codex |
