...
**Decision: P-ATQ-3 accepted within its measured scope** (2026-08-26).
*Evidence:*
...
141 of them wrongly labelled `keyword_ability` on funny/token products and
44 correctly. The funny set type is not excluded by protocol S4; these errors
remain a visible bounded limitation even though none occurs in
expansion/core/commander products. *Residual adjudication:* Urza's Science Fair
Project #0:2 is a correct prevention body inside a die-roll result row (with
its boundary question separate); Khârn the Betrayer #0:2 and Diamond Weapon
#0:2 are correct flavor-word prevention statics under CR 207.2d and 615.1a.
*What would reverse it:* a defect in the prefix rule itself (as opposed to the
...
`docs/current-state.md`, and the bounded limitations register.
...
Legends is **not started**. Its accepted Antiquities prerequisite is complete:
P-ATQ-1..4 are accepted, the five P-ATQ-1 rows are re-annotated, and fresh
exports and metrics have drift 0. D19 (the unscoped `When …` class) remains a
